...
小焦是把大模型当底座、自己套了一层人格和工具链的本地 AI 助手。你可以让它聊天、联网、记东西，也能让它帮你建文件、写网页、跑命令。**兼容 DeepSeek Harness 社区插件**——功能型插件可移植接入小焦，界面型插件在 DSH 里用、小焦当模型，支持 `dsh-plugin` 生态。下面把它的用法、原理、玩法都给你列全。
