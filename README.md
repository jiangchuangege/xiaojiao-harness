...
| 版本 | 内容 |
| --- | --- |
| **v1.2.0（当前）** | 🛠️ **稳定化落地版**：📊 指标与观测（`/metrics` + JSON + 落盘）· 🪵 统一日志与**全链路脱敏** · 🧹 **会话自动回收**（TTL / 空闲 / LRU 上限）· ⚡ **批量并发可配置**（跨域并发实测提速 85%）· 🧪 **压力测试进仓库 + CI 每天跑**（通过率 <95% 直接失败）· 📐 `ARCHITECTURE.md` / `CONTRIBUTING.md` / 发版回滚预案 · 🧭 图语法自检；并修掉 103 处静默吞异常、JSON 展示混乱、依赖清单两处致命错误 |
| v1.1.0 | 🕷️ **内置 Scrapling 抓取**（原生 13 工具 1:1 + 3 增强 = 17 工具：网页/动态页/接口/批量/登录态/下载任意文件 + 抓完自动解读 + 用户使用时学习）+ 🧠 **小脑改为必需项**（路径不写死、全盘自动探测）+ 🛠️ 安装器检测分级（必需/可选分离）+ 🐱 猫娘询问式启动 + 一批渲染/加载器/熔断等修复 |
