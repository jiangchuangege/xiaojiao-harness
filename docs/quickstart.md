...
- **建新会话**：左边「＋ 新对话」。
- **🐱 N.E.K.O. 猫娘（可选）**：启动时**先问你一句**「是否同时启动猫娘宠物」→ 答 `y` 才拉起 Live2D 桌面猫娘（`main_server(48911)` + `memory_server(48912)`）+ 后台学你与猫娘的对话；答 `n` 或非交互式**不拉，也不影响小焦**（`XIAOJIAO_NEKO_AUTO=1` 免询问）。
- **💰 成本看板**：浏览器打开 `http://127.0.0.1:5000/cost`。
- **📷 视觉识图**：设 `XIAOJIAO_VISION_URL=http://127.0.0.1:8082/v1`，猫娘拍照即识图。

## 装小焦体检

猫娘点「🛠️ 装小焦」→ 秒出环境清单 ✅/❌，告诉你缺什么、怎么装。
