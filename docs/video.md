...
点 🎬 → llama-swap 卸载大脑(9292, 秒级) → 启动 ComfyUI(8188) → Wan2.1-FP8 生成(480p)
     → 下载视频到本地 videos/ → 温存15分钟(聊天上显卡时不杀,连续视频秒级)/切第三脑或闲置超时冷 → 网页播放
```
...
4. 系统**自动**：卸大脑→起 ComfyUI→生成(约2-3分钟)→**温存15分钟**(聊天上显卡时不杀,连续视频秒级)/切第三脑或闲置超时自动释放。
...
- **ComfyUI(8188)**：生成完温存 15 分钟（聊天上显卡时不杀,连续视频秒级）；切第三个大脑/闲置超时自动停止释放内存。
