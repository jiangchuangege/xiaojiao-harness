...
# ============ 路径解析（自动检测 + 配置覆盖） ============
# 为什么要自动检测：别人的机器上 Archify 装在哪都可能（全局 npm / dsh profile / 自备 vendor 目录），
# 插件里写死绝对路径 = clone 下来直接用不了。解析顺序：
#   ① 控制文件 xiaojiao_control.json 的 scrapling.archify_root（最高优先，显式指定）
#   ② 环境变量 ARCHIFY_ROOT
#   ③ 常见安装位置自动扫描
# 三层都落空 → 打印**可读**提示（告诉用户怎么装/怎么填），插件其余功能照常注册，
# 让用户能从 doctor 工具里看到原因，而不是插件静默不加载。
_ARCHIFY_CANDIDATES = (
    os.path.expanduser("~/.dsh/profiles/web/node_modules/@tt-a1i/archify-dsh/skills/archify"),
    os.path.expanduser("~/.dsh/profiles/default/node_modules/@tt-a1i/archify-dsh/skills/archify"),
    os.path.expanduser("~/AppData/Roaming/npm/node_modules/@tt-a1i/archify-dsh/skills/archify"),
    os.path.join(os.getcwd(), "vendor", "archify"),
    os.path.expanduser("~/.dsh/node_modules/@tt-a1i/archify-dsh/skills/archify"),
    "/usr/local/lib/node_modules/@tt-a1i/archify-dsh/skills/archify",
    "/usr/lib/node_modules/@tt-a1i/archify-dsh/skills/archify",
)
ARCHIFY_NOT_FOUND_HINT = (
    "未找到 Archify。请执行 `npm install -g @tt-a1i/archify-dsh`，"
    "或在 xiaojiao_control.json 的 `scrapling.archify_root` 填写安装路径"
    "（示例：C:\\Users\\你的用户名\\.dsh\\profiles\\web\\node_modules\\@tt-a1i\\archify-dsh\\skills\\archify）。"
)
_ARCHIFY_CACHE: Dict[str, Any] = {"root": "", "why": ""}


def _cfg_archify_root() -> str:
    """从控制文件读 scrapling.archify_root（读不到就返回空串）。"""
    try:
        p = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                         "xiaojiao_control.json")
        if os.path.exists(p):
            with open(p, encoding="utf-8") as f:
                c = json.load(f)
            v = ((c or {}).get("scrapling") or {}).get("archify_root") or ""
            return str(v).strip()
    except Exception:  # noqa: silent-ok — 配置读不到就走自动检测
        pass
    return ""


def _looks_like_archify(p: str) -> bool:
    """这个目录是不是 Archify 安装根（以 bin/archify.mjs 为准）。"""
    return bool(p) and os.path.exists(os.path.join(p, "bin", "archify.mjs"))


def _resolve_archify_root(force: bool = False) -> str:
    """解析 Archify 安装根目录，结果缓存。

    严格按三层优先级（**配置填了就以配置为准，不再自动兜底** —— 否则用户填错路径时
    会被自动检测悄悄"纠正"，他永远不知道自己填错了，换台机器就又坏了）：
      ① 控制文件 `scrapling.archify_root`（填了就只看它）
      ② 环境变量 `ARCHIFY_ROOT`
      ③ 常见安装位置自动扫描
    都落空 → 返回空串并打印可读提示。
    """
    if _ARCHIFY_CACHE["root"] and not force:
        return _ARCHIFY_CACHE["root"]
    cfg = _cfg_archify_root()
    if cfg:
        if _looks_like_archify(cfg):
            _ARCHIFY_CACHE.update({"root": cfg, "why": "配置 scrapling.archify_root"})
            LOG.info("Archify 安装位置：%s（来源：配置 scrapling.archify_root）", cfg)
            return cfg
        _ARCHIFY_CACHE.update({"root": "", "why": "配置里的路径无效"})
        LOG.warning("配置 scrapling.archify_root = %r 下没找到 bin/archify.mjs。%s",
                    cfg, ARCHIFY_NOT_FOUND_HINT)
        return ""
    env = (os.environ.get("ARCHIFY_ROOT", "") or "").strip()
    if _looks_like_archify(env):
        _ARCHIFY_CACHE.update({"root": env, "why": "环境变量 ARCHIFY_ROOT"})
        LOG.info("Archify 安装位置：%s（来源：环境变量 ARCHIFY_ROOT）", env)
        return env
    for cand in _ARCHIFY_CANDIDATES:
        if _looks_like_archify(cand):
            _ARCHIFY_CACHE.update({"root": cand, "why": "自动检测"})
            LOG.info("Archify 安装位置：%s（来源：自动检测）", cand)
            return cand
    _ARCHIFY_CACHE.update({"root": "", "why": "未找到"})
    LOG.warning(ARCHIFY_NOT_FOUND_HINT)
    return ""


ARCHIFY_ROOT = _resolve_archify_root()          # 启动解析一次并缓存
ARCHIFY_BIN = os.path.join(ARCHIFY_ROOT, "bin", "archify.mjs") if ARCHIFY_ROOT else ""
SCHEMAS_DIR = os.path.join(ARCHIFY_ROOT, "schemas") if ARCHIFY_ROOT else ""
EXAMPLES_DIR = os.path.join(ARCHIFY_ROOT, "examples") if ARCHIFY_ROOT else ""
SKILL_MD = os.path.join(ARCHIFY_ROOT, "SKILL.md") if ARCHIFY_ROOT else ""

# 输出目录跟着**项目目录**走（插件文件所在目录的上一层），不再写死盘符路径
